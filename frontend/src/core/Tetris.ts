...
export abstract class Tetris {
    ...
    protected async StartNextRound() {
        ...
        if (!isSuccess) return await this.handleGameOver(true)
    ...
    */
    ...
    public async handleGameOver(hasLost: boolean): Promise<void> {
        clearInterval(this.currInterval)
    ...
    public async handleFriendLeft(): Promise<void> {
        clearInterval(this.currInterval)
        ...
        this.textStatus.innerText = "Your friend has left the game."
