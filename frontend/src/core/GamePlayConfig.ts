...
export class Game extends Tetris {
    ...
    constructor(mainArea: HTMLElement, pauseScreen: HTMLElement, continueButton: HTMLElement, textStatus: HTMLElement) {
        ...
        this.pauseButton = document.getElementById('pauseButton') as HTMLElement
    ...
    private async setupFriendMode(gameId: string) {
        ...
        const { startWebSocketConnection, joinRoom, setupWebSocketGameListeners } = await this.importSignalRServices()
    ...
    private handlePauseButtonClick = async () => {
        this.togglePause()
    ...
    private async importSignalRServices() {
        ...
        ]).then(([signalR, signalRSenders, signalRPreGame]) => ({
            ...signalR,
            ...signalRSenders,
            ...signalRPreGame
        }))
